...
const TeamMemberCard = ({ name, role, description, icon: Icon, gradient, onClick }: TeamMemberCardProps) => {
  return (
    ...
    >
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="px-8 pb-8 relative z-10">
          <motion.div
            ...
            className="flex items-center justify-center px-4 py-2 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors group-hover:border-neutral-500/30"
          >
            <Info className="h-5 w-5 text-neutral-400 mr-2" />
      ...
      {/* Hover effect */}
      <motion.div 
        className="absolute inset-0 bg-gradient-to-br from-neutral-500/5 to-indigo-500/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl"
        initial={false}
