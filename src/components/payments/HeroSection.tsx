  ...
  return (
    <section className="pt-32 pb-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-black to-gray-900/20">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <span className="inline-block px-4 py-1 rounded-full bg-neutral-900/30 border border-neutral-500/30 text-neutral-300 text-sm font-medium mb-4">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Button 
              size="lg" 
              className="bg-gradient-to-r from-neutral-600 to-indigo-600 hover:from-neutral-700 hover:to-indigo-700"
            >
              Get Started Now
            </Button>
            <Button 
        ...
        >
          {[
            { icon: <CreditCard className="h-6 w-6 text-neutral-400" />, title: "200+ Currencies", text: "Accept payments in local currencies worldwide" },
            { icon: <Lock className="h-6 w-6 text-neutral-400" />, title: "Automatic Compliance", text: "Global tax and regulatory compliance built-in" },
            { icon: <Globe className="h-6 w-6 text-neutral-400" />, title: "Global Reach", text: "Scale your business to new markets instantly" },
            { icon: <Zap className="h-6 w-6 text-neutral-400" />, title: "Instant Setup", text: "Go live with payments in minutes" }
          ].map((item, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, y: 20 }}
