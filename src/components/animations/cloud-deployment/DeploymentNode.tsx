      ...
      <div className={`flex h-14 w-14 items-center justify-center rounded-lg ${getNodeBackgroundClass()}`}>
        ...
        {node.status && (
          <div className="absolute -right-1 -bottom-1 rounded-full bg-gray-800 border border-gray-700 p-0.5">
            {node.status === 'deployed' && <Check className="h-3 w-3 text-neutral-400" />}
            {node.status === 'pending' && (
              <motion.div 
                animate={{ rotate: 360 }}
                transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
              >
                <svg className="h-3 w-3 text-neutral-400" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {node.status === 'error' && <X className="h-3 w-3 text-neutral-400" />}
          </div>
        )}
      </div>
