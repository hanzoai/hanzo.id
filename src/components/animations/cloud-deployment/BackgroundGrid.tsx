  ...
  return (
    <>
      ...
      {scanPoints.map((point, idx) => (
        <motion.div
          key={`point-${idx}`}
          className="absolute h-1 w-1 rounded-full bg-neutral-500/10" // Reduced opacity
          style={{ 
            left: `${point.x}%`, 
            top: `${point.y}%`,
      ...
      {connectionPairs.map((pair, idx) => (
        <React.Fragment key={`connection-${idx}`}>
          ...
          <div
            className="absolute h-px bg-neutral-400/5" // Reduced opacity
            style={{
              left: `${pair.from.x}%`,
              top: `${pair.from.y}%`,
          ...
          <motion.div
            className="absolute h-1 w-1 rounded-full bg-neutral-400/30" // Smaller and reduced opacity
            style={{
              left: `${pair.from.x}%`,
              top: `${pair.from.y}%`,
